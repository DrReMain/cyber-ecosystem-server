...
type QueryMenuByRoleCodeLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
	logx.Logger
}

func NewQueryMenuByRoleCodeLogic(ctx context.Context, svcCtx *svc.ServiceContext) *QueryMenuByRoleCodeLogic {
	return &QueryMenuByRoleCodeLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
...
func (l *QueryMenuByRoleCodeLogic) QueryMenuByRoleCode(in *admin_system.MenuListByRoleCodeReq) (*admin_system.MenuListRes, error) {
	items, err := l.svcCtx.DB.Menu.Query().
		Where(menu.HasRolesWith(role.CodeIn(in.RoleCode...))).
	...
	for _, v := range items.List {
		m[v.ID] = &admin_system.MenuBody{
			...
			Resources:  []*admin_system.ResourceBody{},
