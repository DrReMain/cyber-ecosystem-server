...
func (_q *MenuQuery) QueryResources() *ResourceQuery {
...
func (_q *MenuQuery) WithResources(opts ...func(*ResourceQuery)) *MenuQuery {
	query := (&ResourceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *MenuQuery) loadResources(ctx context.Context, query *ResourceQuery, nodes []*Menu, init func(*Menu), assign func(*Menu, *Resource)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Menu)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
