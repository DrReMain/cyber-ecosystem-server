...
import (
	...
	"github.com/DrReMain/cyber-ecosystem-server/rpc/admin_system/ent/migrate"
...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		c.Department, c.Menu, c.Position, c.Resource, c.Role, c.User,
	} {
		n.Use(hooks...)
	}
}
...
func (c *MenuClient) QueryResources(_m *Menu) *ResourceQuery {
...
func (c *ResourceClient) QueryMenu(_m *Resource) *MenuQuery {
	query := (&MenuClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		Department, Menu, Position, Resource, Role, User []ent.Hook
	}
	inters struct {
		Department, Menu, Position, Resource, Role, User []ent.Interceptor
	}
)
